...
if (isset($_GET["surveyor_id"])) {
  ...
  $sql = "SELECT DISTINCT contract_area FROM parcels_lookup";
  if($q = mysql_query($sql)) { 
    if (mysql_num_rows($q)) {
      ...
      if($s = mysql_query($sql)) { 
        ...
        } else print "Error! This surveyor is not assigned any blocks."; #No blocks assigned to surveyor
      } else print "Error! MySQL returned error <pre>".mysql_error()."</pre>";
      
      # Let's make the simplest page 
      print "<p> Surveyor: <b> $surveyor_name </p>";
      print "<form action='./select_block.php' method='get'>\n";
      print "<input type='hidden' name='surveyor_id' value='$surveyor_id'>\n";
      print "Select Area: <select name='area_id'>\n  "; 
      while ($row = mysql_fetch_assoc($q)) { 
        print "<option>".$row["contract_area"]."</option>\n"; 
      } 
      print "</select> <br><br> <td><input type='submit' value='Submit'>\n</form></td></br>";
    } else print "Error! No parcel areas available."; 
  } else print "Error! MySQL returned error <pre>".mysql_error()."</pre>";
